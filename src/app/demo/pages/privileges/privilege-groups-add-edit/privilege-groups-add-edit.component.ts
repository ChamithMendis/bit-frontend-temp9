...
export class PrivilegeGroupsAddEditComponent {
  ...
  public onFormSubmit(): void {
    try {
      ...
      if (this.data) {
        this._privilegesService.editPrivilegeGroup(this.data.id, this.privilegeGroupForm.value).then((response: any) => {
          console.log(response);
          this._dialogRef.close(true);
        });
      } else {
        this._privilegesService.addPrivilegeGroup(this.privilegeGroupForm.value).then((response: any) => {
          console.log(response);
          // handle response
          this._dialogRef.close(true);
        });
